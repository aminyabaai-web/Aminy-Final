...
-- [MIGRATION FIX] Table vault_documents already created in earlier migration
...
-- 4. Vault Share Links
-- [MIGRATION FIX] Table vault_share_links already created in earlier migration
-- Original CREATE TABLE commented out to prevent silent column loss:
...
-- [MIGRATION FIX] Table vault_access_log created in earlier migration.
-- Adding columns that would have been lost due to IF NOT EXISTS:
-- Original CREATE TABLE commented out below.
ALTER TABLE vault_access_log ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT now();
...
-- [MIGRATION FIX] Table calm_tool_sessions already created in earlier migration
-- Original CREATE TABLE commented out to prevent silent column loss:
...
-- 7. Daily Routines
-- [MIGRATION FIX] Table daily_routines already created in earlier migration
-- Original CREATE TABLE commented out to prevent silent column loss:
...
-- Adding columns that would have been lost due to IF NOT EXISTS:
-- Original CREATE TABLE commented out below.
ALTER TABLE routine_completions ADD COLUMN IF NOT EXISTS child_id UUID;
ALTER TABLE routine_completions ADD COLUMN IF NOT EXISTS scheduled_date DATE NOT NULL DEFAULT CURRENT_DATE;
ALTER TABLE routine_completions ADD COLUMN IF NOT EXISTS started_at TIMESTAMPTZ;
ALTER TABLE routine_completions ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'in_progress', 'completed', 'skipped', 'partial'));
ALTER TABLE routine_completions ADD COLUMN IF NOT EXISTS steps_completed INTEGER DEFAULT 0;
ALTER TABLE routine_completions ADD COLUMN IF NOT EXISTS total_steps INTEGER DEFAULT 0;
ALTER TABLE routine_completions ADD COLUMN IF NOT EXISTS adherence_score INTEGER DEFAULT 0 CHECK (adherence_score BETWEEN 0 AND 100);
...
ALTER TABLE routine_completions ADD COLUMN IF NOT EXISTS challenges_noted JSONB DEFAULT '[]';
