...
const STATUS_CONFIG: Record<BenefitStatusValue, {
  ...
  'submitted':   { icon: Clock,         color: '#4795AE', bg: 'rgba(106,169,188,0.12)', label: 'Submitted' },
  'review':      { icon: Clock,         color: '#E0A45F', bg: 'rgba(224,164,95,0.12)',  label: 'In review' },
  'approved':    { icon: CheckCircle,   color: '#2A7D99', bg: 'rgba(42,125,153,0.12)',  label: 'Approved ✓' },
  ...
  return (
    <div style={{ fontFamily: 'Manrope, system-ui, sans-serif' }}>
      ...
      {stateConfig && (
        ...
        >
          <div style={{ fontSize: 11, fontWeight: 700, color: '#2A7D99', letterSpacing: '0.06em', marginBottom: 8 }}>
          ...
          {stateConfig.ddAgency.phone && (
            <a
              ...
              style={{
                ...
                marginTop: 10, fontSize: 13, fontWeight: 600, color: '#2A7D99',
                textDecoration: 'none',
              }}
            >
              <Phone size={13} />
      ...
      {stateConfig?.waiver.selfDirected && (
        ...
        >
          <Star size={15} color="#2A7D99" style={{ flexShrink: 0, marginTop: 2 }} />
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#2A7D99', marginBottom: 3 }}>
            ...
            {stateConfig.keyResources.map(r => (
              <a
                ...
                style={{
                  ...
                  fontSize: 13, color: '#4795AE', fontWeight: 500,
                  textDecoration: 'none',
                  padding: '8px 12px',
                  background: '#fff',
                  border: '1px solid #E8E4DF',
    ...
    <div style={{ fontFamily: 'Manrope, system-ui, sans-serif' }}>
      {stateConfig && (
        <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: '#2A7D99', background: 'rgba(42,125,153,0.10)', borderRadius: 100, padding: '2px 10px' }}>
            {stateConfig.medicaid.name}
          </span>
          <span style={{ fontSize: 12, fontWeight: 600, color: '#4795AE', background: 'rgba(106,169,188,0.10)', borderRadius: 100, padding: '2px 10px' }}>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        {approved > 0 && (
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#2A7D99' }}>{approved}</div>
            <div style={{ fontSize: 11, color: '#8A9BA8' }}>Approved</div>
          </div>
        )}
        {inProgress > 0 && (
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#4795AE' }}>{inProgress}</div>
...
function InfoRow({ icon: Icon, label, sublabel, accent = false }: {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <Icon size={13} color={accent ? '#2A7D99' : '#8A9BA8'} style={{ flexShrink: 0 }} />
      <span style={{ fontSize: 12, color: '#8A9BA8' }}>{sublabel}: </span>
      <span style={{ fontSize: 12, fontWeight: 600, color: accent ? '#2A7D99' : '#3A4A57' }}>
        {label}
      </span>
    </div>
  );
