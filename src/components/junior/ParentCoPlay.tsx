    ...
    return (
      <div className="flex flex-col h-full bg-[#F8F8F6] p-4">
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-sm text-[#5A6B7A] mb-6">Great work with {childName}</p>
          ...
          <div className="w-full max-w-xs mb-6">
            ...
            <div className="flex gap-3 mt-2 text-sm text-[#5A6B7A]">
          ...
          <div className="w-full max-w-xs mb-4">
            ...
            <textarea
              ...
              className="w-full p-3 rounded-xl border border-[#E8E4DF] text-sm resize-none h-20 focus:outline-none focus:ring-2 focus:ring-[#2A7D99]/30"
            />
          </div>
          ...
          <div className="flex gap-3 w-full max-w-xs">
            <button
              ...
              className="flex-1 py-3 bg-[#2A7D99] text-white rounded-xl font-semibold text-sm active:scale-95 transition-transform"
            >
              Save & Finish
            </button>
            <button
  ...
  return (
    <div className={`flex flex-col h-full bg-[#F8F8F6] ${isSplitMode ? '' : 'relative'}`}>
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-[#E8E4DF]">
        ...
        >
          {isPaused ? <Play className="w-4 h-4 text-[#2A7D99]" /> : <Pause className="w-4 h-4 text-[#5A6B7A]" />}
        </button>
      </div>
          ...
          >
            <div className="bg-white rounded-2xl p-6 text-center max-w-[280px]">
              ...
              <button
                ...
                className="w-full py-2.5 bg-[#2A7D99] text-white rounded-xl font-semibold text-sm active:scale-95 transition-transform"
              >
                Resume
              </button>
            </div>
      ...
      <div className="px-4 py-2 bg-white">
        <div className="flex items-center gap-2">
          <div className="flex-1 h-2 bg-[#EDF4F7] rounded-full overflow-hidden">
            <motion.div
              className="h-full bg-[#2A7D99] rounded-full"
              initial={false}
              animate={{ width: `${((currentTrial - 1) / totalTrials) * 100}%` }}
              transition={{ duration: 0.3 }}
            />
          </div>
          <span className="text-sm font-medium text-[#5A6B7A]">{accuracy}%</span>
        </div>

        {/* Mini stats */}
        <div className="flex justify-between mt-1.5 text-sm text-[#5A6B7A]">
      ...
      {isSplitMode && (
        <div className="flex-1 min-h-0 p-4 flex items-center justify-center">
          <div className="bg-white rounded-2xl p-6 shadow-sm w-full max-w-xs text-center">
            ...
            <div className="text-4xl font-bold text-[#2A7D99]">{currentTrial}</div>
      ...
      <div className={`${isSplitMode ? '' : 'flex-1'} bg-white rounded-t-2xl shadow-lg border-t border-[#E8E4DF] p-4`}>
            ...
            >
              <div className="flex items-start gap-2 bg-amber-50 rounded-xl p-3 mb-3">
                ...
                <div className="flex-1">
                  <div className="text-sm font-medium text-amber-800">{currentTip}</div>
                </div>
                <button onClick={() => setShowTips(false)} className="text-amber-400 text-sm">hide</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="mb-3">
          ...
          >
            ...
            {showPromptDetails ? <ChevronUp className="w-3 h-3 text-[#5A6B7A]" /> : <ChevronDown className="w-3 h-3 text-[#5A6B7A]" />}
          </button>
              ...
              <button
                ...
                className={`flex-1 flex flex-col items-center gap-1 py-2 rounded-xl text-sm transition-all active:scale-95 ${
                  selectedPromptLevel === level
                    ? 'bg-[#2A7D99] text-white shadow-sm'
              ...
              >
                <div className="mt-2 p-3 bg-[#F6FBFB] rounded-xl">
                  {PROMPT_LEVELS.map(({ level, label, description, icon }) => (
                    <div
                      key={level}
                      className={`flex items-center gap-2 py-1.5 ${level === selectedPromptLevel ? 'text-[#2A7D99] font-medium' : 'text-[#5A6B7A]'}`}
                    >
                      {icon}
                      <div>
        ...
        <div className="flex items-center justify-between">
          <button
            ...
            className="flex items-center gap-1 text-sm text-[#5A6B7A] active:scale-95"
          >
          ...
          {trials.length > 0 && (
            <button
              ...
              className="flex items-center gap-1 text-sm text-[#E07A5F] active:scale-95"
            >
