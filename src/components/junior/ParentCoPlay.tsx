    ...
    return (
      <div className="flex flex-col h-full bg-[#F8F8F6] p-4">
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-sm text-[#577590] mb-6">Great work with {childName}</p>

          {/* Stats Grid */}
          <div className="grid grid-cols-2 gap-3 w-full max-w-xs mb-6">
            <div className="bg-white rounded-xl p-3 shadow-sm">
              <div className="text-2xl font-bold text-[#43AA8B]">{accuracy}%</div>
              <div className="text-xs text-[#577590]">Accuracy</div>
            </div>
            <div className="bg-white rounded-xl p-3 shadow-sm">
              <div className="text-2xl font-bold text-[#43AA8B]">{independentCount}/{trials.length}</div>
              <div className="text-xs text-[#577590]">Independent</div>
            </div>
            <div className="bg-white rounded-xl p-3 shadow-sm">
              <div className="text-2xl font-bold text-[#577590]">{avgPromptLabel}</div>
              <div className="text-xs text-[#577590]">Avg Prompt</div>
            </div>
            <div className="bg-white rounded-xl p-3 shadow-sm">
              <div className="text-2xl font-bold text-[#577590]">{formatTime(elapsedSeconds)}</div>
              <div className="text-xs text-[#577590]">Duration</div>
            </div>
          </div>
          ...
          <div className="w-full max-w-xs mb-6">
            ...
            <div className="flex gap-3 mt-2 text-xs text-[#577590]">
          ...
          <div className="flex gap-3 w-full max-w-xs">
            ...
            <button
              onClick={onExit}
              className="px-4 py-3 bg-gray-100 text-[#577590] rounded-xl font-medium text-sm active:scale-95 transition-transform"
            >
              Discard
            </button>
          </div>
  ...
  return (
    <div className={`flex flex-col h-full bg-[#F8F8F6] ${isSplitMode ? '' : 'relative'}`}>
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-gray-100">
        <button
          onClick={onExit}
          className="flex items-center gap-1 text-sm text-[#577590] active:scale-95 transition-transform"
        >
        ...
        <div className="text-center">
          ...
          <div className="text-xs text-[#577590]">
        ...
        <button
          ...
          className="p-1.5 rounded-lg bg-gray-100 active:scale-95 transition-transform"
        >
          {isPaused ? <Play className="w-4 h-4 text-[#43AA8B]" /> : <Pause className="w-4 h-4 text-[#577590]" />}
        </button>
      </div>
        ...
        {isPaused && (
          ...
          >
            <div className="bg-white rounded-2xl p-6 text-center max-w-[280px]">
              <Pause className="w-8 h-8 text-[#577590] mx-auto mb-3" />
              <h3 className="font-bold text-[#0D1B2A] mb-1">Session Paused</h3>
              <p className="text-sm text-[#577590] mb-4">Take a break. {childName} is doing great!</p>
              <button
      ...
      <div className="px-4 py-2 bg-white">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-[#577590]">{accuracy}%</span>
        </div>

        {/* Mini stats */}
        <div className="flex justify-between mt-1.5 text-xs text-[#577590]">
      ...
      {isSplitMode && (
        <div className="flex-1 min-h-0 p-4 flex items-center justify-center">
          <div className="bg-white rounded-2xl p-6 shadow-sm w-full max-w-xs text-center">
            ...
            {activityDescription && (
              <p className="text-sm text-[#577590] mb-3">{activityDescription}</p>
            )}
            <div className="text-4xl font-bold text-[#43AA8B]">{currentTrial}</div>
            <div className="text-xs text-[#577590] mt-1">Current Trial</div>
          </div>
        </div>
      )}
      ...
      <div className={`${isSplitMode ? '' : 'flex-1'} bg-white rounded-t-2xl shadow-lg border-t border-gray-100 p-4`}>
        ...
        <div className="mb-3">
          ...
          >
            ...
            {showPromptDetails ? <ChevronUp className="w-3 h-3 text-[#577590]" /> : <ChevronDown className="w-3 h-3 text-[#577590]" />}
          </button>
              ...
              >
                <div className="mt-2 p-3 bg-gray-50 rounded-xl">
                  {PROMPT_LEVELS.map(({ level, label, description, icon }) => (
                    <div
                      key={level}
                      className={`flex items-center gap-2 py-1.5 ${level === selectedPromptLevel ? 'text-[#43AA8B] font-medium' : 'text-[#577590]'}`}
                    >
                      {icon}
                      <div>
        ...
        <div className="mb-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <button
              ...
              className="flex flex-col items-center gap-1.5 py-3 bg-gray-50 rounded-xl text-gray-500 active:scale-95 active:bg-gray-100 transition-all"
            >
        ...
        <div className="flex items-center justify-between">
          <button
            ...
            className="flex items-center gap-1 text-xs text-[#577590] active:scale-95"
          >
