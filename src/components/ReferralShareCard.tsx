...
export function ReferralShareCard({ referralCode, userName = 'A friend' }: ReferralShareCardProps) {
    ...
    return (
        <motion.div
            ...
            className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-[#E8E4DF] dark:border-slate-700 p-6 sm:p-8 max-w-md w-full"
        >
            <div className="flex items-center justify-center w-16 h-16 rounded-full bg-[#EEF4F8] dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 mb-6 mx-auto">
            ...
            <div className="bg-[#F6FBFB] dark:bg-slate-900 rounded-xl p-4 flex items-center justify-between mb-8">
                ...
                <Button
                    ...
                    size="icon"
                    onClick={handleCopy}
                    className={copied ? 'text-green-500' : 'text-slate-400 hover:text-[#5A6B7A] dark:hover:text-slate-300'}
                >
