...
export function ScreenHeader({
  ...
  const variantClasses = {
    flat:     'bg-white border-b border-slate-100',
  ...
  return (
    <div className={`${baseClasses} ${variantClasses} ${stickyClasses} ${className}`}>
      {onBack && (
        <button
          onClick={onBack}
          className="flex items-center gap-1 text-sm text-slate-600 mb-3 hover:text-slate-900 transition-colors min-h-[36px]"
        >
      ...
      <div className="flex items-start gap-3">
        {icon && (
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-white shrink-0" style={{ background: 'linear-gradient(135deg, #43AA8B 0%, #577590 100%)' }}>
            {icon}
          </div>
        )}
