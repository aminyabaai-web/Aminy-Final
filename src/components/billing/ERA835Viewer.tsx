...
function LineStatusBadge({ status }: { status: PaidClaimLine['claimStatus'] }) {
  switch (status) {
    ...
    default:
      return (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-[#F0EDE8] text-gray-600">
  ...
  return (
    <div className={`rounded-xl border mb-2 overflow-hidden ${rowBg}`}>
      ...
      <div className="px-4 py-3">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex items-center gap-2">
            <span className="font-mono font-semibold text-sm text-gray-800">{line.procedureCode}</span>
            <LineStatusBadge status={line.claimStatus} />
          </div>
          <span className="text-xs text-gray-400">{formatDate(line.serviceDate)}</span>
        </div>

        {/* Amounts grid */}
        <div className="grid grid-cols-4 gap-2 text-center">
          {[
            { label: 'Billed', value: line.billedAmount, cls: 'text-gray-700' },
      ...
      {expanded && hasAdjustments && (
        <div className="border-t border-gray-100 bg-white/80 px-4 py-3 space-y-1.5">
          {line.adjustments.map((adj, i) => (
            <div key={i} className="flex items-start justify-between gap-2 text-xs">
              <div className="flex-1 min-w-0">
                <span className={`font-medium mr-1 ${
                  adj.adjustmentGroupCode === 'CO' ? 'text-orange-600' :
                  adj.adjustmentGroupCode === 'PR' ? 'text-blue-600' : 'text-gray-600'
                }`}>
                  {adj.adjustmentGroupCode}-{adj.reasonCode}
                </span>
                <span className="text-gray-500 truncate">{adj.description}</span>
              </div>
      ...
      <Card className="p-5 rounded-2xl border-0 shadow-sm bg-white">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-semibold text-gray-800 text-sm leading-tight">ERA Remittance</h2>
              <p className="text-xs text-gray-400">Check #{era.checkNumber}</p>
            </div>
          </div>
