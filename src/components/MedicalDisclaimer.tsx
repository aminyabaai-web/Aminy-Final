...
export function MedicalDisclaimer({ variant = 'footer', className = '' }: MedicalDisclaimerProps) {
  if (variant === 'inline') {
    return (
      <p className={`text-sm text-[#5A6B7A] dark:text-[#8A9BA8] ${className}`}>
  ...
  return (
    <div className={`text-center py-4 px-4 ${className}`}>
      <p className="text-sm text-[#5A6B7A] dark:text-[#8A9BA8] max-w-2xl mx-auto">
      ...
      <p className="text-sm text-[#8A9BA8] dark:text-[#5A6B7A] mt-2">
