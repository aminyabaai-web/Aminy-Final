...
export function CoachModeComplete({ activity, onClose, onComplete, nextActivity }: CoachModeProps) {
  const [currentState, setCurrentState] = useState<'overview' | 'active' | 'paused' | 'completion'>('overview');
  ...
  const handlePauseResume = () => {
    ...
    setCurrentState(isPaused ? 'active' : 'paused');
    toast.info(isPaused ? 'Activity resumed' : 'Activity paused');
  };
  ...
  const handleReflection = (mood: 'great' | 'okay' | 'challenging') => {
    ...
    toast.success('Thank you for sharing!', {
      description: 'Your feedback helps us improve activities',
      duration: 3000
    });
  };
  ...
  return (
    <>
      {/* Backdrop - Fixed positioning to prevent layout shifts */}
      <div 
      ...
      {/* Coach Mode Card - 8px radius, proper touch targets, explicit dimensions */}
      <div 
        className="fixed inset-4 md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-2xl md:w-full bg-white rounded-[8px] shadow-2xl z-50 flex flex-col max-h-[90vh]"
        style={{ 
      ...
      >
        ...
        {currentState === 'overview' && (
          <>
            ...
            {/* Steps List - Numbered 1, 2, 3... - Reserve space to prevent shifts */}
            <div 
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-[16px]">Step {step.id}</span>
                    </div>
            ...
            {/* Control Buttons - 16px padding, 8px radius, 44px touch targets */}
            <div className="p-6 border-t border-gray-200 space-y-3">
              <div className="flex gap-3">
                <Button
                  variant="outline"
                  onClick={handlePrint}
                  className="flex-1 min-h-[44px] py-4 px-4 rounded-[8px] focus:ring-2 focus:ring-accent focus:ring-offset-2"
                >
                ...
                <Button
                  ...
                  className="flex-1 min-h-[44px] py-4 px-4 rounded-[8px] bg-accent hover:bg-accent/90 focus:ring-2 focus:ring-accent focus:ring-offset-2"
                >
        ...
        {(currentState === 'active' || currentState === 'paused') && currentStepData && (
          <>
            {/* Header with Timer */}
            <div className="flex items-start justify-between p-6 border-b border-gray-200">
              <div className="flex-1">
            ...
            {/* Current Step Highlighted - Reserve space to prevent layout shifts */}
            <div 
            ...
            >
              <div className="p-6 rounded-[8px] border-2 border-accent bg-accent/5 mb-4">
                ...
                <p className="text-[16px] text-gray-900 leading-relaxed mb-4">
              ...
              {/* Voice Prompts Section - "What do I say?" with Mic icon */}
              {currentStepData.voicePrompts && currentStepData.voicePrompts.length > 0 && (
                <div className="mb-4">
                  <Button
                    variant="outline"
                    onClick={() => setShowVoicePrompts(!showVoicePrompts)}
                    className="w-full min-h-[44px] justify-between rounded-[8px] focus:ring-2 focus:ring-accent focus:ring-offset-2"
                  >
                  ...
                  {showVoicePrompts && (
                    <div className="mt-2 p-4 bg-blue-50 border border-blue-200 rounded-[8px] space-y-2">
                      {currentStepData.voicePrompts.map((prompt, idx) => (
                        <div key={idx} className="flex items-start gap-2">
                          ...
                          <p className="text-[16px] text-blue-900">{prompt}</p>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {/* Try This Instead Section - Live alternatives */}
              {currentStepData.alternatives && currentStepData.alternatives.length > 0 && (
                <div className="mb-4">
                  <Button
                    variant="outline"
                  ...
                  {showAlternatives && (
                    <div className="mt-2 p-4 bg-purple-50 border border-purple-200 rounded-[8px] space-y-2">
                      {currentStepData.alternatives.map((alt, idx) => (
                        <div key={idx} className="flex items-start gap-2">
                          <Sparkles className="w-4 h-4 text-purple-600 flex-shrink-0 mt-0.5" />
                          <p className="text-[16px] text-purple-900">{alt}</p>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className="px-3 py-2 bg-white border border-gray-300 rounded-[8px] text-sm hover:border-accent hover:bg-accent/5 transition-all focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:outline-none"
                  >
                    😅 Too hard
                  </button>
                  <button
            ...
            {/* Control Buttons - Start/Pause/Skip with 16px padding */}
            <div className="p-6 border-t border-gray-200 space-y-3">
              <div className="flex gap-3">
                <Button
                  variant="outline"
                ...
                <Button
                  ...
                  disabled={isPaused}
                  className="flex-1 min-h-[44px] py-4 px-4 rounded-[8px] bg-accent hover:bg-accent/90 disabled:opacity-50 focus:ring-2 focus:ring-accent focus:ring-offset-2"
                >
              ...
              <button
                ...
                disabled={isPaused}
                className="w-full min-h-[44px] text-sm text-muted-foreground hover:text-accent transition-colors disabled:opacity-50 focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:outline-none rounded-[8px]"
              >
                Skip this step
              </button>
            </div>
        ...
        {/* SCREEN 3: COMPLETION with celebration animation */}
        {currentState === 'completion' && (
          <>
            {/* Celebration Header with animation */}
            <div className="p-6 border-b border-gray-200 text-center">
              <div className="mb-4 flex justify-center relative">
                {/* Main checkmark with zoom-in animation */}
                <div className="relative animate-in zoom-in-50 duration-500">
                  ...
                  <PartyPopper className="w-6 h-6 text-amber-500 absolute -bottom-1 -left-1 animate-bounce" />
                </div>
              </div>
              <h1 className="text-3xl font-semibold text-primary mb-2 animate-in fade-in duration-700">
                Great job!
              </h1>
              <p className="text-[16px] text-muted-foreground animate-in fade-in duration-700 delay-100">
                You completed {activity.title} in {formatTime(elapsedTime)}
              </p>
            </div>

            {/* Reflection Section - 3 emoji reactions - Reserve space */}
            <div 
            ...
            >
              <div>
                ...
                <div className="flex gap-3 justify-center">
                  ...
                  <button
                    ...
                    className={`flex-1 p-6 rounded-[8px] border-2 transition-all min-h-[44px] focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:outline-none ${
                      reflection === 'great'
                  ...
                  >
                    ...
                    <div className="font-medium text-[16px]">Great</div>
                  </button>
                  ...
                  <button
                    ...
                    className={`flex-1 p-6 rounded-[8px] border-2 transition-all min-h-[44px] focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:outline-none ${
                      reflection === 'okay'
                        ? 'border-gray-500 bg-gray-50 scale-105'
                        : 'border-gray-200 hover:border-gray-300 hover:scale-105'
                    }`}
                  >
                    <div className="text-4xl mb-2">😐</div>
                    <div className="font-medium text-[16px]">Okay</div>
                  </button>
                  ...
                  >
                    <div className="text-4xl mb-2">😞</div>
                    <div className="font-medium text-[16px]">Challenging</div>
                  </button>
                </div>
              </div>
              ...
              {nextActivity && (
                <div>
                  ...
                  <div className="p-4 rounded-[8px] border-2 border-gray-200 bg-white hover:border-accent transition-all cursor-pointer">
                    <div className="flex items-start justify-between mb-2">
                      <h4 className="font-semibold text-[16px]">{nextActivity.title}</h4>
                    ...
                    <Button 
                      size="sm" 
                      className="w-full min-h-[44px] rounded-[8px] bg-accent hover:bg-accent/90 focus:ring-2 focus:ring-accent focus:ring-offset-2"
                    >
                      Start now
                    </Button>
                  </div>
            ...
            {/* Action Buttons - "Done for now" button */}
            <div className="p-6 border-t border-gray-200 space-y-3">
              <Button
                onClick={onComplete}
                className="w-full min-h-[44px] py-4 px-4 rounded-[8px] bg-accent hover:bg-accent/90 focus:ring-2 focus:ring-accent focus:ring-offset-2"
              >
                Back to Plan
              </Button>
              <button
                onClick={onClose}
                className="w-full min-h-[44px] text-[16px] text-muted-foreground hover:text-accent transition-colors focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:outline-none rounded-[8px]"
              >
                Done for now
              </button>
            </div>
...
export function CoachModeCompleteExample() {
  ...
  return (
    ...
    >
      <div className="max-w-md mx-auto">
        ...
        {/* Card with explicit min-height to prevent CLS */}
        <div 
          className="bg-white border border-gray-200 rounded-[8px] p-4 shadow-sm"
          style={{ 
            minHeight: '200px',
            contain: 'layout style'
          }}
        >
          ...
          <Button
            onClick={() => {
              setIsLoading(true);
              // Simulate loading to prevent instant layout shift
              setTimeout(() => {
                setShowCoachMode(true);
                setIsLoading(false);
              }, 100);
            }}
            disabled={isLoading}
