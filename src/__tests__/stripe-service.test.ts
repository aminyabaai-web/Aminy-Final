...
describe('Stripe Service', () => {
  describe('calculateVisitPrice', () => {
    it('should return base price for non-member without promo', () => {
      const result = calculateVisitPrice('consult', false);
    ...
    it('should apply member discount', () => {
      const result = calculateVisitPrice('consult', true);
    ...
    it('should apply promo code discount', () => {
      const result = calculateVisitPrice('consult', false, 'WELCOME20');

      expect(result.discount).toBeGreaterThan(0);
      expect(result.total).toBeLessThan(result.subtotal);
    });

    it('should stack member and promo discounts', () => {
      const result = calculateVisitPrice('consult', true, 'WELCOME20');
    ...
    it('should not go below zero', () => {
      ...
      const result = calculateVisitPrice('follow-up', true, 'AMINY50');

      expect(result.total).toBeGreaterThanOrEqual(0);
    });

    it('should calculate extended session correctly', () => {
      const result = calculateVisitPrice('extended', false);
  ...
  describe('validatePromoCode', () => {
    it('should validate WELCOME20 code', () => {
    ...
    it('should validate FIRST10 code', () => {
      const result = validatePromoCode('FIRST10');

      expect(result.valid).toBe(true);
      expect(result.description).toBe('$10 off');
    });

    it('should validate AACT25 partner code', () => {
      const result = validatePromoCode('AACT25');
    ...
    it('should be case insensitive', () => {
      const upper = validatePromoCode('WELCOME20');
      const lower = validatePromoCode('welcome20');
      const mixed = validatePromoCode('Welcome20');
