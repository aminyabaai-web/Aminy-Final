...
- ✅ **make-server deployed (v163, July 2)** — wins-append verified live via functional probe July 3. CommunicationTemplates "Send SMS/Email" buttons are WIRED (session-auth fetches to `/notifications/sms` + `/email/provider-message`; both routes live in make-server; demo-mode short-circuits). Verified July 15.
