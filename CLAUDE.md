...
- ✅ **Sentry DSN — CONFIGURED.** Real DSN baked into `netlify.toml` `[build.environment]` (and `.github/workflows/e2e-playwright.yml`), so every prod build inlines `VITE_SENTRY_DSN`; `initSentry()` runs in `src/main.tsx`. Session replays mask all text/inputs/media (HIPAA). No further action.
...
- **⏰ OWNER REMINDER (due ~July 11, 2026):** deploy `make-server-8a022548` — server-side AI-memory fixes (wins-append, dedup/expiry, crisis-safety prompts) merged June 2 wave are NOT live until this ships. From a repo checkout: `supabase functions deploy make-server-8a022548 --project-ref qpzsvafwcwyrkdolrjbu` (or owner pastes a fresh sbp_ token in chat). REMIND EDGAR if date passed.
